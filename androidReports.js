...
var statusColor = "";
var fileName = 'android-build-data.json';
...
async function getBuildData(){
  ...
  var options = {
    'method': 'GET',
    'url': 'https://api-cloud.browserstack.com/app-automate/flutter-integration-tests/v2/android/builds/'+buildid,
    'headers': {
  ...
  setTimeout(() => {
      ...
      fs.writeFile("android-reports/"+jsonData.id+".html", `
        ...
        if (err) throw err;
        console.log('File created - android-reports/'+jsonData.id+".html");
      });
      ...
      for (var i = 0; i < jsonData.devices.length; ++i) {        
        for(var j = 0; j < jsonData.devices[i].sessions.length; j++){
          ...
          fs.appendFile("android-reports/"+jsonData.id+".html", `
          
                  <p><strong>Device: `+jsonData.devices[i].device+`</strong></p>
                  <p>Session ID: <a href='https://app-automate.browserstack.com/dashboard/v2/builds/`+jsonData.id+`/sessions/tests/`+jsonData.devices[i].sessions[j].id+`df9d19cd'>`+jsonData.devices[i].sessions[j].id+`</a></p>
